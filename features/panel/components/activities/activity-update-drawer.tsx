...
export type ActivityUpdateDrawerProps = { data: ActivityType }

export default function ActivityUpdateDrawer({ data }: ActivityUpdateDrawerProps) {
  const [open, setOpen] = useState<boolean>(false)
  const queryClient = useQueryClient()

  const form = useForm<ActivityUpdateSchemaProps>({
    mode: 'onSubmit',
  ...
  return (
    <Drawer open={open} onOpenChange={(open) => setOpen(open)}>
      ...
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>Update your activity</DrawerTitle>
          <DrawerDescription>Do the thing</DrawerDescription>
        </DrawerHeader>
        ...
        <DrawerFooter className="mx-auto max-w-3xl max-sm:w-full sm:flex-row">
          ...
          <SubmitButton onClick={form.handleSubmit(onSubmit)} loading={isPendingUpdateActivity}>
            Save
          </SubmitButton>
        </DrawerFooter>
      </DrawerContent>
